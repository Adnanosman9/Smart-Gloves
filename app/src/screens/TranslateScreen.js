...
export function TranslateScreen() {
  const { leftGlove, rightGlove, status, count, scan, disconnect } = useBleGloves();
  ...
  const handleConnect = useCallback(async () => {
    if (status === 'connected' || status === 'partial') await disconnect();
    else await scan();
  }, [status, scan, disconnect]);
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.gloveStatusRow}>
          ...
          <View style={styles.gloveStatusCenter}>
            <Text style={styles.gloveStatusCount}>{count}/2</Text>
